...
const utils = {
  ...
  openGithub: () =>
    shell.openExternal("https://github.com/DuanNguyen2907/TikDouyin_Down.git"),
...
function prepareI18n(lang) {
  const i18n = {
    ...
    get: (item, ...args) => {
      return (i18n[i18n.lang][item] || item).replace(
        /\{(\d+)\}/g,
...
async function initApp() {
  ...
  eventList.forEach((item) =>
    ipcRenderer.on(item, (event, data) => ipc.eventStore[item](data))
  );
}
